               ...
               seed = NULL)
{
    ...
    if (optim)
    {
        ...
        optimArgs.default$control[names(optimArgs$control)] <-
            optimArgs$control
    ...
    {
        if (parallel & !vectorize)
        {
            parallel <- startParallel(parallel)
            stopCluster <- TRUE
        } else if (parallel & vectorize) {
    ...
    stud <- matrix(as.double(NA), nrow = 1, ncol = nvars)
    studFitness <- -Inf # dummies for bookeeping
    ...
    stud <- object@stud <- Pop[1,] # set the first individual as the stud
    studFitness <- object@studFitness <- do.call(fitness, c(list(Pop[1,]), callArgs)) # evaluate its fitness
    # start iterations
    for (iter in seq_len(maxiter))
    {
        ...
        if (!parallel & !vectorize) # PLAIN SEQUENTIAL IMPLEMENTATION
        {
        ...
        } else if (vectorize & !parallel) { # VECTORIZED IMPLEMENTATION
            if (nvars > 1) {
            Fitness[is.na(Fitness)] <- apply(X = Pop[is.na(Fitness),], 
                                             MARGIN = 1, 
                                             function(x) do.call(fitness, c(list(x), callArgs)))
            } else {
                Fitness[is.na(Fitness)] <- apply(X = matrix(Pop[is.na(Fitness),], ncol = 1), 
                                                 MARGIN = 1, 
                                                 function(x) do.call(fitness, c(list(x), callArgs)))
            }
            if (updatePop) {
                Pop <- apply(Fitness, 1, function(x) attributes(x)[[1]])
            }
        }  else if (vectorize & parallel) { # PARALLEL VECTORIZED IMPLEMENTATION USING FUTURE.APPLY
            if (nvars > 1) {
                Fitness[is.na(Fitness)] <- future_apply(X = Pop[is.na(Fitness),], 
                                                 MARGIN = 1, 
                                                 function(x) do.call(fitness, c(list(x), callArgs)))
            } else {
                Fitness[is.na(Fitness)] <- future_apply(X = matrix(Pop[is.na(Fitness),], ncol = 1), 
                                                 MARGIN = 1, 
                                                 function(x) do.call(fitness, c(list(x), callArgs)))
            }
            if (updatePop)
                Pop <- future_apply(Fitness, 1, function(x) attributes(x)[[1]])
        ...
        {
            object@bestSol[[iter]] <-
                unique(Pop[Fitness == max(Fitness, na.rm = TRUE), , drop = FALSE])
        }
        ...
        if (studLogic) {
            if (is.na(object@studFitness) | FitnessSorted[1] > object@studFitness) {
                object@stud <- stud <- PopSorted[1,]
                object@studFitness <- studFitness <- FitnessSorted[1]
            } else { # it means that the population lost its stud somehow... replace an ugly duckling with the stud
                # idx <- which(Fitness == Fitness[Fitness < object@studFitness & !is.na(Fitness)][1], arr.ind = TRUE)[1]
                # if (!is.na(idx)) {
                #     Pop[idx, ] <- object@stud
                #     Fitness[idx] <- object@studFitness
                # }
            }
        }
        ...
        #stdidx <- sample(1:popSize, 1)
        if (studLogic) { # randomly, meh, replace one individual by the stud
            
            Pop[1, ] <- object@stud
            Fitness[1] <- object@studFitness
        }
        object@population <- Pop
        object@fitness <- Fitness
        ...
        if (is.function(crossover) & pcrossover > 0) {
            ...
            if (studLogic) { # eventually could be numeric argument to randomly have N matings with stud
                mating[,1] <- 1 # always have the stud as the first parent
            }
            if (!vectorize) {
                ...
                mated <- runif(nmating)
                Crossover <- vector(mode = "list", length = nrow(mating[mated < pcrossover, ])) # preallocate
                Crossover <- apply(mating[mated < pcrossover, ], 1, function(x) crossover(object, x) )
                Pop[mating[mated < pcrossover], ] <- matrix(unlist(lapply(Crossover, function(x) x$children)), ncol = nvars)
                Fitness[mating[mated < pcrossover]] <- unlist(lapply(Crossover, function(x) x$fitness))
            }
            # if (studLogic) { # randomly, meh, replace one individual by the stud
            #     
            #     Pop[1, ] <- object@stud
        ...
        if (is.function(mutation) & pm > 0) {
            if (!vectorize) {
                ...
                mutated <- seq(1:popSize)[runif(popSize) < pm]
                Mutation <- vector(mode = "list", length = length(mutated))
                Mutation <- lapply(mutated, function(i) mutation(object, i))
                Pop[mutated,] <- matrix(unlist(Mutation), ncol = nvars)
                Fitness[mutated] <- NA
            }
            # if (studLogic) { # randomly, meh, replace one individual by the stud
            #     
        ...
        # if we have studlogic, we have to make sure to keep the stud
        if (studLogic) {
            # find the first individual in the population with a lower fitness than the stud
            # and replace it by the stud
            idx <- which(Fitness == Fitness[Fitness < object@studFitness & !is.na(Fitness)][1], arr.ind = TRUE)[1]
            if (!is.na(idx)) {
                Pop[idx, ] <- object@stud
                Fitness[idx] <- object@studFitness
    ...
    if (keepBest)
        object@bestSol <-
        object@bestSol[!sapply(object@bestSol, is.null)]
...
setClassUnion("numericOrMatrix", members = c("numeric", "matrix"))
setClass(
    Class = "ga",
    representation(
        call = "language",
        ...
        solution = "matrix",
        stud = "numericOrMatrix",
        studFitness = "numericOrNA"
    ),
    package = "GA"
)

setMethod("print", "ga", function(x, ...)
    str(x))
...
summary.ga <- function(object, ...)
{
    ...
    out <- list(
        ...
        stud    = object@stud,
        studFitness = object@studFitness,
...
print.summary.ga <- function(x, digits = getOption("digits"), ...)
{
    ...
    cat(paste("Stud Logic            = ", is.na(x$stud), "\n"))
    cat(paste(
                    ...
                    ...)
{
    ...
    if (missing(ylim))
    {
        ylim <- c(max(apply(summary[, c(2, 4)], 2,
                            function(x)
                                min(range(
                                    x, na.rm = TRUE, finite = TRUE
                                )))),
                  max(range(
                      summary[, 1], na.rm = TRUE, finite = TRUE
                  )))
    }
