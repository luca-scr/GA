...
Futher details are provided in Scrucca (2013) and Scrucca (2016). See also `help(package="GA")` for a list of available functions and methods. 
...
```
The code above defines a vector of `workers` containing the IP address for each node of the cluster. This is used by 
`makeCluster()` to create a *PSOCK Snow cluster* object named `cl`.
...
At this point a `ga()` function call can be executed by providing the argument `parallel = cl`. At the end of GA iterations the cluster is automatically stopped by shutting down the workers.
